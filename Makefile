...
CFLAGS := $(CFLAGS) -Weverything -Wno-gcc-compat -Wno-implicit-int -Wno-deprecated-non-prototype -Wno-reserved-identifier -Wno-comment -Wno-unsafe-buffer-usage -Wno-long-long -Werror
LFLAGS ?=
...
build: clean
	...
	LTO="$$(clang -flto $(CFLAGS) -Wno-everything -Wno-error "$(SOURCE)/$(NAME)/main.c" -o /dev/null -ferror-limit=1 -fuse-ld=lld $(LFLAGS) 2>/dev/null >/dev/null && echo -flto)"
	STATIC="$$(clang -static $(CFLAGS) -Wno-everything -Wno-error "$(SOURCE)/$(NAME)/main.c" -o /dev/null -ferror-limit=1 -fuse-ld=lld $(LFLAGS) 2>/dev/null >/dev/null && echo -static)"
